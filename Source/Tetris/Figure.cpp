...
void AFigure::NotifyActorBeginOverlap(AActor* OtherActor)
{
	AFigure* OtherFigure = Cast<AFigure>(OtherActor);

	if (OtherFigure)
	{
		for (auto OtherBlock : OtherFigure->GetBlocks())
		{
			FVector OtherLoc = OtherBlock->GetComponentLocation();

			for (auto Block : Blocks)
			{
				FVector Loc = Block->GetComponentLocation();

				// If any block lands on another, call end game from gamemode
				if (FMath::IsNearlyEqual(OtherLoc.X, Loc.X, EqualityTolerance) && FMath::IsNearlyEqual(OtherLoc.Y, Loc.Y, EqualityTolerance))
				{
